...
interface MatchCardProps {
  match: Match;
  // Optional pre-resolved team/venue data — pages that already fetched
  // teams/venues from the backend (e.g. Home, Matches) pass these in so
  // MatchCard doesn't re-fetch per card. When omitted, this falls back
  // to the local mock lookup so the component still works standalone.
  teamA?: Team;
  teamB?: Team;
  venue?: VenueSummary;
}

const MatchCard: React.FC<MatchCardProps> = ({ match, teamA: teamAProp, teamB: teamBProp, venue: venueProp }) => {
  const navigate = useNavigate();
  const { setConfig } = useMatch();
  const teamA = teamAProp ?? getTeamById(match.teamAId);
  const teamB = teamBProp ?? getTeamById(match.teamBId);
  const venue = venueProp ?? getVenueById(match.venueId);
